-- @sdk/libs/settings — pre-game settings screen, 3-button, push-only.
-- Resolve: require("@sdk/libs/settings")
--
...
-- baked assets are the Vietnamese text labels (the HUD's single centred label
-- can't do a positioned layout and Sprite has no text API). If even those are
-- missing the lib degrades to a single multi-line HUD label.
--
...
function M.new(spec)
  ...
  local self = setmetatable({
    ...
    kit         = spec.kit or "@sdk/images/settings",
...
function Menu:_build()
  ...
  place(sp.bg, 0, 0)
  if HUD and HUD.set_label then HUD.set_label("") end
...
-- Text fallback (no Sprite.solid / kit missing): single multi-line HUD label.
function Menu:_render_text()
  ...
  if HUD and HUD.set_label then HUD.set_label(table.concat(lines, "\n")) end
end
